...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            int i = 0;
            const char sign[] = " ";
            char* sp[3] = {"NULL", "NULL", "NULL"}; 
            sp[0] = sp[method];
            sp[1] = sp[requestTarget];
            sp[2] = sp[version];
            sp[method] = strtok(line, sign);
             
          
            while (sp[i] != NULL && i < 2)
            {
                i++;
                sp[i] = strtok(NULL , sign);
         }
            char* HTTPVersion = strtok(sp[version],"\n\r");      
            if (strcmp(sp[method], "GET") != 0)
            {
                error(405);
                continue;
    }
    else if (sp[requestTarget][0] != '/')
            {
                error(501);
                continue;
    }   
            else if (strchr(sp[requestTarget], '\"') != NULL)
            {
            ...
            char* queryNull = strchr(sp[requestTarget], '\0');
            char* checkMark = strchrnul(sp[requestTarget], '?');
            char query[(queryNull - checkMark < 1) ? 1 : queryNull - checkMark ];
            memcpy(query, (strncmp(checkMark , "\0", 1) == 0) ? 
            checkMark : checkMark + 1,(queryNull - checkMark < 1) ? 1 : queryNull - checkMark );                      
            checkMark = strchrnul(sp[requestTarget], '?');
            char absolutePath[checkMark - sp[requestTarget] + 1];
            strncpy(absolutePath, sp[requestTarget], checkMark - sp[requestTarget]);
            absolutePath[checkMark - sp[requestTarget]] = '\0';         
            char* checkDoT = strchr(absolutePath, '.');
            if (checkDoT == NULL)
            {
                error(501);
                continue;
            ...
            checkMark = strchr(absolutePath, '\0');
            char extension[checkMark - checkDoT];
            memcpy(extension, checkDoT + 1, checkMark - checkDoT);
